...
function generateTotalRankingsBySport() {
  ...
  results.forEach(r => {
    // 将原始sportType映射到四大项目
    const sportType = sportTypeMergeMap[r.sportType] || r.sportType;
  ...
  // 获取所有合并后的项目类型（四大项目）
  const sportTypes = [...new Set(Array.from(athleteStatsMap.values()).map(s => s.sportType))];
  console.log(`项目类型: ${sportTypes.map(st => sportTypeNames[st] || st).join(', ')}`);
  ...
  // 合并所有项目的排名（兼容旧格式）
  const allRankings = sportRankingsList.flatMap(sr => sr.rankings)
    .sort((a, b) => b.totalPoints - a.totalPoints)
    .map((r, i) => ({ ...r, rank: i + 1 }));
