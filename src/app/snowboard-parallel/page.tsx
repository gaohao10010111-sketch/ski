...
export default function SnowboardParallelPage() {
  ...
  const rankings = useMemo(() => {
    const allAthletes: { rank: number; name: string; team: string; points: number; discipline: string; ageGroup: string; gender: string; time?: string }[] = [];

    for (const competition of latestResults.competitions) {
      if (competition.sportType !== 'snowboard-parallel') continue;
  ...
  const topAthletes = useMemo(() => {
    ...
    const result: { id: number; name: string; nation: string; discipline: string; points: string; worldRank: number; wins: number }[] = [];
    for (let idx = 0; idx < sorted.length; idx++) {
      const a = sorted[idx];
      const rank = (idx > 0 && a.points === sorted[idx - 1].points) ? result[idx - 1].worldRank : idx + 1;
      result.push({ id: rank, name: a.name, nation: '中国', discipline: a.discipline, points: a.points.toString(), worldRank: rank, wins: a.events });
    }
    return result;
  }, []);
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {rankings.map((athlete, idx) => (
                        <tr key={idx} className={`hover:bg-gray-50 ${idx < 3 ? 'bg-gradient-to-r from-yellow-50/50 to-transparent' : ''}`}>
                          ...
                          <td className="px-4 py-3 whitespace-nowrap">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium ${idx < 3 ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-800'}`}>
                              {athlete.points} 分
                            </span>
                          </td>
                        </tr>
        ...
        {parallelCompetitions.length > 0 && (
          <section className="mb-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/competitions" className="text-indigo-600 hover:text-indigo-700 text-sm font-medium flex items-center">
                查看全部 <ChevronRight className="w-4 h-4 ml-1" />
              </Link>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {parallelCompetitions.map((comp) => (
                <Link key={comp.competition} href="/competitions" className="bg-white rounded-lg p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow block">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold text-indigo-600">{comp.events.reduce((sum, e) => sum + e.athletes.length, 0)}人次</span>
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <section className="h-full">
              <div className="bg-white rounded-lg p-6 shadow-sm h-full">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link href="/athletes" className="text-indigo-600 hover:text-indigo-700 text-sm font-medium">
                    全部运动员 →
                  </Link>
                </div>
