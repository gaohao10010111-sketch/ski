...
export default function AlpinePage() {
  ...
  const rankings = useMemo(() => {
    ...
    for (const competition of latestResults.competitions) {
      if (competition.sportType !== 'alpine') continue;
  ...
  // 从真实数据生成历史冠军 - 使用 useMemo 缓存（不再限制数量）
  ...
  const championsPerPage = 8; // 每页显示8个冠军
                            ...
                            <div className="flex items-center">
                              <div className="flex-shrink-0 h-8 w-8">
                                <div className="h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center">
                                  <span className="text-xs font-medium text-blue-600">{athlete.name.charAt(0)}</span>
                                </div>
                              </div>
                              <div className="ml-3">
