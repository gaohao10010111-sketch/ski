...
export default function CompetitionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchCompetition = async () => {
      ...
      setError(null)
      try {
        const response = await competitionsApi.get(resolvedParams.id)
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex gap-3">
            {competition.status === 'COMPLETED' && (
              <Link
                href={`/results-announcement?competition=${encodeURIComponent(competition.name)}&id=${resolvedParams.id}`}
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {competition.results.slice(0, 20).map((result: CompetitionResult) => {
                      const statusInfo = resultStatusLabels[result.status] || { label: result.status, color: 'text-gray-600' }
                      return (
